...
static bool push(WorkList* const work, HashSet* const visited, StgClosure * const tagged) {
  ...
  if(hs_bccprelude_hashset_member(visited, untagged)) {
    return true;
  } else {
    return hs_bccprelude_worklist_push(work, untagged);
  }
}
...
static StgClosure const * const pop(WorkList* const work, HashSet* const visited, unsigned int* const err) {
  StgClosure const * const p = hs_bccprelude_worklist_pop(work);

  if(p != NULL) {
    Inserted inserted = hs_bccprelude_hashset_insert(visited, p);
    switch(inserted) {
 ...
 */
 uint64_t hs_bccprelude_closureSize(unsigned int const workListCapacity, unsigned int const visitedInitCapacity, unsigned int const visitedMaxCapacity, unsigned int* const err, StgStablePtr const sp) {
    ...
    *err = NO_FAILURE;
    ...
    WorkList* work = hs_bccprelude_worklist_alloc(workListCapacity);
    if(work == NULL) {
      *err = OUT_OF_MEMORY;
      return 0;
    }
    ...
    HashSet* visited = hs_bccprelude_hashset_alloc(visitedInitCapacity, visitedMaxCapacity);
    if(visited == NULL) {
      *err = OUT_OF_MEMORY;
      hs_bccprelude_worklist_free(work);
      return 0;
    }
    ...
    hs_bccprelude_worklist_free(work);
    hs_bccprelude_hashset_free(visited);
