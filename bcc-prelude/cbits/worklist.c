 ...
 */
WorkList* hs_bccprelude_worklist_alloc(unsigned int capacity) {
  ...
  HashSet* set = hs_bccprelude_hashset_alloc(capacity, capacity);
  if(set == NULL) {
    free(work);
    free(stack);
    return NULL;
 ...
 */
bool hs_bccprelude_worklist_push(WorkList* const work, void const * const p) {
  unsigned int count = work->wlHashSet->hsCount;

  Inserted inserted = hs_bccprelude_hashset_insert(work->wlHashSet, p);
  switch(inserted) {
 ...
 */
void const * const hs_bccprelude_worklist_pop(WorkList* const work) {
  unsigned int count = work->wlHashSet->hsCount;

  if(count > 0) {
    void const * const p = work->wlStack[count - 1];
    work->wlStack[count - 1] = NULL; // to aid debugging
    hs_bccprelude_hashset_delete(work->wlHashSet, p);
    return p;
  } else {
    return NULL;
  }
 ...
 */
void hs_bccprelude_worklist_free(WorkList* const work) {
  hs_bccprelude_hashset_free(work->wlHashSet);
  free(work->wlStack);
  free(work);
}
