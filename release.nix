...
# The project sources
{ bcc-prelude ? { outPath = ./.; rev = "abcdef"; }

# Function arguments to pass to the project
, projectArgs ? {
    config = { allowUnfree = false; inHydra = true; };
    gitrev = bcc-prelude.rev;
  }
...
# Import pkgs, including bcc common nix lib
, pkgs ? import ./nix { inherit sourcesOverride; }

}:

with (import pkgs.bccNix.release-lib) {
  inherit pkgs supportedSystems supportedCrossSystems scrubJobs projectArgs;
  packageSet = import bcc-prelude;
  gitrev = bcc-prelude.rev;
};
...
let
  ...
  jobs = {
    ...
    #musl64 = mapTestOnCross musl64 (packagePlatformsCross project);
      ...
      jobs.native.haskellPackages.bcc-prelude.components.library.x86_64-linux
      jobs.native.haskellPackages.bcc-prelude.components.library.x86_64-darwin
    ]));
